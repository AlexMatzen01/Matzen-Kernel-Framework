    ...
    loop {
        ...
        match ev.key {
                        ...
                        if answer {
                            ...
                            if let Some(name) = target {
                                match buf.save_as(&name) {
                                    Ok(n) => {
                                        new_status = Some((
                                            alloc::format!("Wrote {} bytes to {}", n, name),
                                            false,
                                        ));
                                    }
                                    Err(e) => {
                                        ...
                                        needs_redraw = true;
                                        status_msg = new_status.map(|(m, _)| {
                                            (m, true, crate::shell::get_tick_count() + 4000)
                                        });
                                        draw_and_position(&buf, &mut vp, &status_msg);
                                        continue;
                                    }
                                ...
                                // No filename – prompt
                                if let Some(fname) =
                                    ui::prompt_with_input("File Name to Write:", None)
                                {
                                    if !fname.is_empty() {
                                        match buf.save_as(&fname) {
                                            ...
                                            Err(e) => {
                                                new_status =
                                                    Some((alloc::format!("Error: {}", e), true));
                                                status_error = true;
                                                status_msg = new_status.map(|(m, _)| {
                                                    (m, true, crate::shell::get_tick_count() + 4000)
                                                });
                                        ...
                                        status_msg = new_status.map(|(m, _)| {
                                            (m, false, crate::shell::get_tick_count() + 2000)
                                        });
                                        draw_and_position(&buf, &mut vp, &status_msg);
                                        continue;
                                    }
            ...
            Key::Ctrl('O') | Key::F(3) | Key::Ctrl('S') => {
                ...
                let prompt_default = if default.is_empty() {
                    None
                } else {
                    Some(default.as_str())
                };
                if let Some(fname) = ui::prompt_with_input("File Name to Write:", prompt_default) {
                    if fname.is_empty() {
                        ...
                        match buf.save_as(&fname) {
                            ...
                            Err(e) => {
                                new_status =
                                    Some((alloc::format!("Error writing file: {}", e), true));
                                status_error = true;
                            }
                        }
                    }
            ...
            Key::Ctrl('W') | Key::F(6) => {
                ...
                if let Some(needle) = ui::prompt_with_input("Search:", None) {
                    if !needle.is_empty() {
                        ...
                        let sc = if start_col > buf.lines[start_row].len() {
                            0
                        } else {
                            start_col
                        };
                        let sr = if sc == 0 {
                            (start_row + 1).min(buf.lines.len() - 1)
                        } else {
                            start_row
                        };
            ...
            Key::Ctrl('U') | Key::F(10) => {
                if buf.clipboard.is_empty() {
                    new_status = Some((String::from("Cutbuffer empty"), true));
                    status_error = true;
                } else {
            ...
            Key::Ctrl('C') | Key::F(11) => {
                ...
                let pct = if buf.lines.len() > 0 {
                    (buf.cursor_row * 100) / buf.lines.len()
                } else {
                    0
                };
                new_status = Some((
            ...
            Key::Ctrl('_') | Key::F(13) => {
                ...
                if let Some(inp) = ui::prompt_with_input("Enter line number, column number:", None)
                {
                    ...
                    if line_no >= 1 && line_no <= buf.lines.len() {
                        ...
                        vp.ensure_cursor_visible(&buf);
                        new_status = Some((
                            alloc::format!("Goto past EOF – at line {}", buf.lines.len()),
                            true,
                        ));
                        status_error = true;
                    } else {
                        new_status = Some((String::from("Invalid line number"), true));
                        status_error = true;
                    }
                }
            }
            ...
            Key::Ctrl('R') | Key::F(5) => {
                ...
                if let Some(fname) =
                    ui::prompt_with_input("File to insert [from current dir]:", None)
                {
                    if !fname.is_empty() {
                        if !crate::shell::is_mounted() {
                            ...
                            if let Some(data) = crate::shell::read_file_contents(&fname, &mut dev) {
                                ...
                                new_status =
                                    Some((alloc::format!("Failed to read \"{}\"", fname), true));
                                status_error = true;
                            }
                        }
                    }
            ...
            Key::Ctrl('Z') => {
                // Undo alias also Ctrl+Z
                if buf.undo() {
                    vp.ensure_cursor_visible(&buf);
                    new_status = Some((String::from("Undone"), false));
                } else {
                    new_status = Some((String::from("Nothing to undo"), true));
                    status_error = true;
                }
            }
            Key::Ctrl('Y') => {
                if buf.redo() {
                    ...
                    new_status = Some((String::from("Nothing to redo"), true));
                    status_error = true;
                }
            }
            Key::Char(c) if ev.alt && (c == 'u' || c == 'U') => {
                if buf.undo() {
                    vp.ensure_cursor_visible(&buf);
                    new_status = Some((String::from("Undone (Alt+U)"), false));
                } else {
                    new_status = Some((String::from("Nothing to undo"), true));
                    status_error = true;
                }
            ...
            Key::Char(c) if ev.alt && (c == 'e' || c == 'E') => {
                if buf.redo() {
                    vp.ensure_cursor_visible(&buf);
                    new_status = Some((String::from("Redone (Alt+E)"), false));
                } else {
                    new_status = Some((String::from("Nothing to redo"), true));
                    status_error = true;
                }
            ...
            Key::Ctrl('^') | Key::Ctrl('6') => {
                // Ctrl+6 is 0x1E -> '^' (also handle '6' for robustness)
                buf.toggle_mark();
                new_status = Some((
                    if buf.mark.is_some() {
                        String::from("Mark Set (^6)")
                    } else {
                        String::from("Mark Unset")
                    },
                    false,
                ));
            }
            Key::Char(c) if ev.alt && (c == 'a' || c == 'A' || c == 'g' || c == 'G') => {
                ...
                if c == 'g' || c == 'G' {
                    // Goto line via Alt+G
                    if let Some(inp) =
                        ui::prompt_with_input("Enter line number, column number:", None)
                    {
                        let parts: Vec<&str> = inp.split(',').collect();
                        let line_no: usize = parts
                            .get(0)
                        ...
                        if line_no >= 1 && line_no <= buf.lines.len() {
                            ...
                            vp.ensure_cursor_visible(&buf);
                            new_status =
                                Some((alloc::format!("Go to line {} (Alt+G)", line_no), false));
                        } else {
                            new_status = Some((String::from("Invalid line number"), true));
                            status_error = true;
                        }
        ...
        if let Some((m, _)) = &new_status {
            ...
            status_msg = Some((
                m.clone(),
                status_error,
                crate::shell::get_tick_count() + 3500,
            ));
...
fn goto_replace(
    buf: &mut TextBuffer,
    vp: &mut Viewport,
    new_status: &mut Option<(String, bool)>,
    ...
    if let Some(needle) = ui::prompt_with_input("Search (to replace):", None) {
        ...
        if let Some(repl) = ui::prompt_with_input("Replace with:", None) {
            ...
            let (srow, scol) = if start_col + 1 <= buf.lines[start_row].len() {
            ...
            if let Some((r, c)) = buf.find_next(needle.as_bytes(), srow, scol) {
                ...
                if let Some(yes) =
                    ui::confirm(&alloc::format!("Replace \"{}\" with \"{}\"?", needle, repl))
                {
                    if yes {
                        buf.replace_next(needle.as_bytes(), repl.as_bytes(), r, c);
                        vp.ensure_cursor_visible(buf);
                        *new_status = Some((
                            alloc::format!("Replaced \"{}\" at {}:{}", needle, r + 1, c + 1),
                            false,
                        ));
                    } else {
