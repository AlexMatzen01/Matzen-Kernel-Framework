...
impl Superblock {
    ...
    pub fn new(total_blocks: u64) -> Self {
        let inode_blocks = (MAX_INODES * core::mem::size_of::<Inode>() + FS_BLOCK_SIZE - 1) / FS_BLOCK_SIZE;
...
impl SimpleFilesystem {
    ...
    pub fn format(device: &mut dyn crate::drivers::block::BlockDevice) -> Result<(), &'static str> {
        ...
        let data_block_start = unsafe { core::ptr::addr_of!(superblock.data_block_start).read_unaligned() };
        ...
        unsafe {
            ...
            core::ptr::copy_nonoverlapping(sb_ptr, buffer.as_mut_ptr(), core::mem::size_of::<Superblock>());
        }
    ...
    pub fn mount(device: &mut dyn crate::drivers::block::BlockDevice) -> Result<Self, &'static str> {
        // Read superblock
        let mut buffer = [0u8; FS_BLOCK_SIZE];
        ...
        for i in 0..superblock.inode_blocks {
            let block_buffer = &mut inode_buffer[(i as usize * FS_BLOCK_SIZE)..((i as usize + 1) * FS_BLOCK_SIZE)];
    ...
    pub fn list_directory(&self, device: &mut dyn crate::drivers::block::BlockDevice, inode_number: u32) -> Result<Vec<FileInfo>, &'static str> {
        ...
        for i in 0..INODE_DIRECT_BLOCKS {
            let block_num = unsafe {
                core::ptr::addr_of!(inode.direct_blocks[i]).read_unaligned()
            };
    ...
    fn find_free_data_block(&self) -> Option<u64> {
        ...
        let start = unsafe { core::ptr::addr_of!(self.superblock.data_block_start).read_unaligned() };
    ...
    fn write_inodes(&self, device: &mut dyn crate::drivers::block::BlockDevice) -> Result<(), &'static str> {
        let inode_bytes = unsafe {
            core::slice::from_raw_parts(
    ...
    fn write_superblock(&self, device: &mut dyn crate::drivers::block::BlockDevice) -> Result<(), &'static str> {
        let mut buffer = [0u8; FS_BLOCK_SIZE];
        unsafe {
    ...
    pub fn create_file(&mut self, device: &mut dyn crate::drivers::block::BlockDevice, filename: &str) -> Result<u32, &'static str> {
        ...
        let dir_block_num = unsafe {
            core::ptr::addr_of!(dir_inode.direct_blocks[0]).read_unaligned()
        };
    ...
    pub fn write_file(&mut self, device: &mut dyn crate::drivers::block::BlockDevice, filename: &str, data: &[u8]) -> Result<(), &'static str> {
        // Find the file
        let files = self.list_directory(device, self.current_dir_inode)?;
        let file_inode_num = files.iter()
    ...
    pub fn read_file(&self, device: &mut dyn crate::drivers::block::BlockDevice, filename: &str) -> Result<Vec<u8>, &'static str> {
        // Find the file
        let files = self.list_directory(device, self.current_dir_inode)?;
        let file_inode_num = files.iter()
            .find(|f| f.name == filename && !f.is_directory)
    ...
    /// Delete a file
    pub fn delete_file(&mut self, device: &mut dyn crate::drivers::block::BlockDevice, filename: &str) -> Result<(), &'static str> {
        // Find the file
        let files = self.list_directory(device, self.current_dir_inode)?;
        ...
        *inode = Inode::new(); // Clear to empty
        ...
        unsafe {
            ...
            core::ptr::write_unaligned(free_blocks_ptr, current_blocks.saturating_add(blocks_used as u64));
        }
